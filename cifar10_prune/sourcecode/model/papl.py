...
def prune_dense(weight_arr, name="None", prune_rate=0.005):
    ...
    thresh = np.percentile(abs(weight_arr), int(100*prune_rate))
    print(name + " current num of 0:", np.sum(abs(weight_arr) == 0))
    print(name + " threshold is:" , thresh)
    ...
    print("Non-zero count (Dense %s): %d,all is %d" % (name, weight_arr.size - count,weight_arr.size))
